...
<div class="dashboardCore">
	...
	<h1 class="flow-text" style="text-align: center;"><b>Product Status</b></h1>
			...
			<table class="highlight data-table" style="font-size: 0.85em;">
		        <thead style="background-color: #508abb; color: #FFFFFF; border-color: #6ea1cc !important;" >
		          <tr class="white-text text-darken">
		              ...
		              <th>Remaining</th>
		              <th>State</th>	
		              <!--prix public algerien use select sum(PPA) from product where-->
		              <th><a href="#" id="editProduct"><b>Modify</b></a></th>
		              <th><a href="#!" id="getRidOfProduct"><b>Delete</b></a></th>
		          </tr>
		        </thead>
		        <?php
		        
		foreach(display_state() as $product)
	{
		?>
		        <tbody>
		        <tr>
		          	<?php 
		          		if($product['Qtt']<10 && $product['Edate']>='20180624'){
				            	$a="Out";
				            }else{
				            	$a="Expired";
				            }
		            ...
		            <td><?=$product['Name']?></td>
		            <td><?=$product['Edate']?></td>
		            <td style="padding-left: 40px;"><?=$product['Qtt']?></td>
...
<div class="updatePostProductManagement">
	...
	<form class="col s12" action="index.php?page=productState" method="POST">
      <div class="rows">
      ...
      <div class="rows">
		...
		<div class="a a1 input-field">
          <input id="qtt" name="Qtt" type="text" class="validate" placeholder="add a Quantity" autocomplete="off">
...
<p>do you want to delate that product? please set the batch numbers <b>Otherwise click on Cancel</b></p>
	<form action="index.php?page=productState" method="POST">
		<div class="row">
				<div class="col s12 m12">
